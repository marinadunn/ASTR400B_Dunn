...
class M33AnalyticOrbit:
    ...
    def __init__(self, filename):
        ...
        self.M33halo = 1.96e11 #IC EDIT: Added  in M33 halo mass
        self.fudge = 1 #IC EDIT: fudge factor
        return None
...
#It returns the acceleration from the Miyamoto-Nagai 1975 potential in the direction of the dummy variable
    ...
    #Its acceleration for the x or y component is the following
        ...
        #print a_k2
        return a_k2
    #IC EDIT: adding in a function that takes in the mass of the satellite (Msat), the postion and velocity coordinates (x,y,z), (vx,vy,vz),
    #and a dummy variable. It will return the acceleration term assuming an isothermal sphere profile for the DM halo of M31.
    def DynamicalFriction(self,Msat,x,y,z,vx,vy,vz,n):
        Vc = 200. #km/s, the typical circular speed at this particular radius in the M31 halo outskirts
...
#the acceleration terms from each galactic component in the direction of dummy variable
    def M31Accel(self,x,y,z,vx,vy,vz,n):
        ...
        DynamicalFriction = self.DynamicalFriction(self.M33halo, x, y, z, vx, vy, vz, n) #IC EDIT: Adding Dynamical Friction to accel sum
        accel_sum = a_bulge + a_halo + a_disk + DynamicalFriction
        return accel_sum
...
#We will update the positions and velocities using standard kinematic equations
    def LeapFrog(self, delta_t, x, y, z, vx, vy, vz): #IC EDIT: Changing to also take in velocity now
        ...
        M31Accel_X = self.M31Accel(x_half, y_half, z_half, vx, vy, vz,'x')
        M31Accel_Y = self.M31Accel(x_half, y_half, z_half, vx, vy, vz, 'y')
        M31Accel_Z = self.M31Accel(x_half, y_half, z_half, vx, vy, vz, 'z')
...
M33Simulation_separation = np.sqrt((M31Orbit['x']-M33Orbit['x'])**2+(M31Orbit['y']-M33Orbit['y'])**2+(M31Orbit['z']-M33Orbit['z'])**2)
